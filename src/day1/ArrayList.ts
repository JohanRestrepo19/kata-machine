export default class ArrayList<T> {
  ...
  removeAt(idx: number): T | undefined {
    ...
    for (let i = idx; i <= this.length; i++) this.array[i] = this.array[i + 1];
    this.length--;
    return value;
  }
  ...
  remove(item: T): T | undefined {
    ...
    if (removedItemIndex !== undefined) return this.removeAt(removedItemIndex);
  ...
  insertAt(item: T, idx: number): void {
    if (idx >= this.capacity) throw new Error("Length has exceded capacity");

    this.length++;

    for (let i = this.length; i > idx; i--) this._array[i] = this._array[i - 1];
